...
import com.yiyuaninfo.Fragment.JJFragment;
...
public class JJActivity extends BaseActivity {
   ...
   // ColorTrackTabLayout colorTrackTabLayout;
    ...
    ViewPager viewPager;
    private List<JJFragment> jjFragmentList = new ArrayList<>();
    private FindFragmentAdapter findFragmentAdapter;
    private TabPageIndicator tabPageIndicator;

    private static final String[] CHANNELS = new String[]{"全部","股票型", "货币型", "债券型"};
    ...
    protected void init(Bundle savedInstanceState) {
       ...
       // colorTrackTabLayout = (ColorTrackTabLayout) findViewById(R.id.ColorTrackTabLayout_jj);
        viewPager = (ViewPager) findViewById(R.id.view_pager_jj);
        tabPageIndicator=(TabPageIndicator)findViewById(R.id.tabPageIndicator_jj);
        setToolBarTitle("基金");

        for (int i = 0; i < ChannelsList.size(); i++) {
            JJFragment fragment = JJFragment.newInstance(ChannelsList.get(i));
            jjFragmentList.add(fragment);
        }
        findFragmentAdapter = new FindFragmentAdapter(getSupportFragmentManager(), jjFragmentList, ChannelsList);
