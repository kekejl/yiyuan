...
import static com.yiyuaninfo.entity.Channeltab.TYPE_MY_CHANNEL;
...
public class ChannelDialogFragment extends DialogFragment implements OnChannelDragListener {
    private List<Channeltab> mDatas = new ArrayList<>();
    ...
    public static ChannelDialogFragment newInstance(List<Channeltab> selectedDatas, List<Channeltab> unselectedDatas) {
    ...
    private void setDataType(List<Channeltab> datas, int type) {
    ...
    private void processLogic() {
        mDatas.add(new Channeltab(Channeltab.TYPE_MY, "我的频道", ""));
        Bundle bundle = getArguments();
        List<Channeltab> selectedDatas = (List<Channeltab>) bundle.getSerializable(ConstanceValue.DATA_SELECTED);
        List<Channeltab> unselectedDatas = (List<Channeltab>) bundle.getSerializable(ConstanceValue.DATA_UNSELECTED);
        setDataType(selectedDatas, TYPE_MY_CHANNEL);
        setDataType(unselectedDatas, Channeltab.TYPE_OTHER_CHANNEL);

        mDatas.addAll(selectedDatas);
        mDatas.add(new Channeltab(Channeltab.TYPE_OTHER, "频道推荐", ""));
        ...
        manager.setSpanSizeLookup(new GridLayoutManager.SpanSizeLookup() {
            ...
            public int getSpanSize(int position) {
                ...
                return itemViewType == TYPE_MY_CHANNEL || itemViewType == Channeltab.TYPE_OTHER_CHANNEL ? 1 : 4;
            }
        });
...
//        if (starPos < 0||endPos<0) return;
    ...
    private void onMove(int starPos, int endPos) {
        Channeltab startChannel = mDatas.get(starPos);
        //先删除之前的位置
